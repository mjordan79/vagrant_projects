#!/usr/bin/env bash
#set -euo pipefail

# Provision a Kubernetes node through MicroK8S and Snapd.
# Maintainer: Renato Perini <renato.perini@gmail.com>

NO_NODE=$1
KUBERNETES_VERSION=$2
ENABLE_RANCHER2_NODE=$3

# User messages are green.
log() {
    echo -e "\e[32m[INFO]\e[0m $*"
}

microk8s_system_info() {
    log "Content of the /etc/hosts file:"
    cat /etc/hosts
}

microk8s_install () {
    log "Installing MicroK8s $1/stable ..."
    snap install microk8s --classic --channel=$1/stable
}

microk8s_certs () {
    log "Insert the load balancer IP after the #MOREIPS line in /var/snap/microk8s/current/certs/csr.conf.template ..."
    sed -i '/^#MOREIPS/a IP.4 = 192.169.0.21' /var/snap/microk8s/current/certs/csr.conf.template
}

microk8s_refresh_interval () {
    log "Replace the line --refresh-interval 30s with --refresh-interval 0s in /var/snap/microk8s/current/args/apiserver-proxy ..."
    sed -i 's/^--refresh-interval 30s/--refresh-interval 0s/g' /var/snap/microk8s/current/args/apiserver-proxy
}

microk8s_iprange () {
    log "Changing --service-cluster-ip-range=10.152.183.0/24 with /16 CIDR in /var/snap/microk8s/current/args/kube-apiserver ..."
    sed -i 's/^--service-cluster-ip-range=10.152.183.0\/24/--service-cluster-ip-range=10.152.183.0\/16/g' /var/snap/microk8s/current/args/kube-apiserver
}

log "MicroK8S: Provisioning MicroK8S v$KUBERNETES_VERSION ..."
microk8s_system_info
microk8s_install $KUBERNETES_VERSION
microk8s status --wait-ready
if [[ "$ENABLE_RANCHER2_NODE" == "false" ]]; then
  if [[ "$NO_NODE" -eq 2 ]] || [[ "$NO_NODE" -eq 3 ]] || [[ "$NO_NODE" -eq 4 ]]; then
    log "MicroK8S: on a Control Plane node. Configuring certs for v.$KUBERNETES_VERSION ..."
    microk8s_certs
    microk8s_refresh_interval
  fi
else
  if [[ "$NO_NODE" -eq 3 ]] || [[ "$NO_NODE" -eq 4 ]] || [[ "$NO_NODE" -eq 5 ]]; then
    log "MicroK8S: on a Control Plane node. Configuring certs for v.$KUBERNETES_VERSION ..."
    microk8s_certs
    microk8s_refresh_interval
  fi
fi

microk8s_iprange
microk8s stop
microk8s start
microk8s status --wait-ready
